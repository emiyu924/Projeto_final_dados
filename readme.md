...
🌐 Links do Projeto
Site (frontend): retro-car-predictor.lovable.app
API (backend): retro-car-predictor.lovable.app
...
Número de revisões
O frontend — acessível em retro-car-predictor.lovable.app — apresenta uma interface retrô com dropdowns e sliders estilizados. Ao clicar em ▶ PREVER PREÇO, os dados são enviados via requisição POST para o backend, que retorna a estimativa de preço calculada por um modelo de Machine Learning.
...
modelo.fit(X, y)
📡 Endpoints da API
A API está disponível em: retro-car-predictor.lovable.app
...
Endpoint: retro-car-predictor.lovable.app
...
Copiar
python app.py
O backend ficará disponível em localhost (ou http://0.0.0.0:8000 na rede local).

🧪 Exemplo de Requisição (cURL)
bash

Copiar
curl -X POST https://retro-car-predictor.lovable.app/prever \
